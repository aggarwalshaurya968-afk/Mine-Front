...
class AIQACog(commands.Cog, name="AI Q&A"):
    ...
    # ── Groq call ────────────────────────────────────────────────────────
    async def _ask_ai(self, question: str, author_name: str) -> str | None:
    ...
    async def on_message(self, message: discord.Message):
        ...
        async with message.channel.typing():
            answer = await self._ask_ai(clean, message.author.display_name)
    ...
    async def aiqa_ask(self, interaction: discord.Interaction, question: str):
        ...
        answer = await self._ask_ai(question, interaction.user.display_name)
        if not answer:
