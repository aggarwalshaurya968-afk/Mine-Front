...
class TierTestModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await create_ticket(self.bot, interaction, f'Tier Test • {self.gamemode} • {self.edition}', answers)
...
class TierPanelView(discord.ui.View):
    ...
    Same checks, same edition/gamemode picker UI, same emojis — only the
    final modal (and therefore the questions asked) differs."""
    ...
    async def _pick_edition(self, interaction: discord.Interaction, edition: str):
        db = self.bot.db
        label = 'Tier Tester applications' if self.kind == 'apply' else 'Tier Test requests'
        ...
        await interaction.response.send_message(
            ...
            view=GamemodeSelectView(self.bot, edition, gamemodes, kind=self.kind),
            ephemeral=True,
        )
...
class TierTestPanelView(discord.ui.View):
    """Separate persistent view (its own custom_ids) for the 'Request a Tier
    Test' panel — visually identical button layout/emojis to TierPanelView,
    just routed to the tier-test modal instead of the staff application one."""

    def __init__(self, bot):
        super().__init__(timeout=None)
        self.bot = bot
        self._delegate = TierPanelView(bot, kind='test')

    @discord.ui.button(label='Java Edition', emoji='🟩',
                       style=discord.ButtonStyle.success, custom_id='tiertest:java')
    async def java(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._delegate._pick_edition(interaction, 'Java Edition')

    @discord.ui.button(label='Bedrock Edition', emoji='🟦',
                       style=discord.ButtonStyle.primary, custom_id='tiertest:bedrock')
    async def bedrock(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self._delegate._pick_edition(interaction, 'Bedrock Edition')
...
async def tier_test_panel_embed(bot, guild: discord.Guild) -> discord.Embed:
    """Same visual layout/emojis as tier_panel_embed — wording changed to make
    ...
    e = discord.Embed(
        title='🧪 Mine Front Tier Test Requests',
        description=(
            f'Want **your** skill tier tested at **{guild.name}**?\n\n'
            'Pick your edition below, then choose the gamemode you want to be tested in '
            'and fill out the request.\n'
            'A Tier Tester will pick it up and get back to you soon.\n\n'
            '━━━━━━━━━━━━━━━━━━━━━━\n'
            f'🟩 **Java Edition gamemodes**{java_closed_note}\n{java_lines}\n\n'
            f'🟦 **Bedrock Edition gamemodes**{bedrock_closed_note}\n{bedrock_lines}\n'
            '━━━━━━━━━━━━━━━━━━━━━━\n\n'
            '> ⚠️ One request per user · Please be patient'
        ),
        color=PURPLE,
        timestamp=datetime.now(timezone.utc)
    )
    if guild.icon:
        e.set_thumbnail(url=guild.icon.url)
        e.set_author(name=f'{guild.name} Tier Test Requests', icon_url=guild.icon.url)
    else:
        e.set_author(name=f'{guild.name} Tier Test Requests')
...
class TierAdminPanelView(discord.ui.View):
    ...
    # ── Row 4: dedicated tier-tester channels ───────────────────────────────
