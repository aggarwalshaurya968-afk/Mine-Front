...
async def record_match_result(bot, guild_id: int, edition: str, gamemode: str,
    ...
    """Applies a win/loss to every member of the winning/losing team. If a new
    tier is supplied it is applied to every member of that team (teammates
    queue individually with their own tier, but a team-result tier bump is
    shared — same as how most big-server team ladders handle it)."""
    await _ensure_tables(bot)
    async with aiosqlite.connect(bot.db.db_path) as db:
...
def result_embed(guild: discord.Guild, match: dict, winner_team: int, score: str,
                  gamemode_emoji: str, changelog: str) -> discord.Embed:
    mode = mode_label(match['team_size'])
    team1_ids = json.loads(match['team1_ids'])
    team2_ids = json.loads(match['team2_ids'])
...
class CompMatchView(discord.ui.View):
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not self._is_participant(interaction.user.id, match) and not await is_admin_or_owner(self.bot, interaction):
            return await interaction.response.send_message(
...
class CompResultModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = result_embed(guild, match, self.winner_team, self.score.value.strip(), emoji, note)
