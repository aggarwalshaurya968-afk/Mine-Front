...
async def create_ticket(bot, interaction: discord.Interaction,
    ...
    # Find or create the Discord category channel for tickets
    ...
    # Ping support roles
    pings = ' '.join(f'<@&{r}>' for r in (settings.get('support_role_ids') or []))
    if pings:
...
class TicketsCog(commands.Cog, name='Tickets'):
    ...
    # ── /setup ─────────────────────────────────────────────────────────────────
    @app_commands.command(name='setup', description='Set up the ticket panel in this channel.')
    ...
    async def add(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not await _has_role(interaction.user,
                               ...
                               settings.get('admin_role_ids', [])):
            return await interaction.response.send_message(
                embed=E.error('You do not have permission to add users.'), ephemeral=True)
    ...
    async def remove(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not await _has_role(interaction.user,
                               ...
                               settings.get('admin_role_ids', [])):
            return await interaction.response.send_message(
                embed=E.error('You do not have permission to remove users.'), ephemeral=True)
